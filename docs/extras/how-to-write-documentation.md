...
Clone [repository](https://gitlab.ics.muni.cz/kypo-crp/kypo-crp-project-docs) from Gitlab. To use local server, go to `kypo-crp-project-docs` and use the following commands: 
```
pipenv sync
...
```
 ...
 and add line in `nav` object as follows: 
```
site_name: MkLorum
nav:
    - Home: index.md
...
Move the cursor above the abbreviation.
...
### Notes
Thanks to the Admonition extension you are able to create notes. Following text: 
```
!!! note
